/*****************************************************************
 ...
 Det [-real, -complex] [-general, -symmertric, -hermitian, -upper_triangular,
  -lower_triangular] [-condition] [-?, ? -h, -help] [-binary_in] [-binary_out]
  [-file_in InputFile] [-file_out OutputFile] [-file_det DeterminantOutputFile]
...
#endif
{
  ...
  FILE     *Ain;
  FILE     *detinv_out[2]; /* the first file ptr is to the det out file     *
  ...
  get_options(argc, argv, &Ain, detinv_out, &find_detinv, &matrix_in, &type);

  flag = 10; /* want the determinant only */
  ...
  if(type) { /* The section for complex matrices */
      ...
      while(TRUE)
	{
	  ...
	  fprintf(stderr,"DeterminantInverse: time taken was %f seconds\n"
		  , elapsed_time);
...
#else
void get_options(argc, argv, Ain, detinv_out, det_fctn, matrix_in, type)
int argc;
char **argv;
FILE **Ain;
FILE **detinv_out;
 ...
 ************************/
{
  ...
  char  *cond_name=NULL,*det_name=NULL;
  int   i;
  int   fact_index;
  int   type_index;
  int   binary_out;
  ...
  *type = 0;
  ...
  for(i=1; i<argc; i++)
	...
	{ 
      ...
      else if(strncmp(argv[i],"-binary_in", 2)==0)
	{
	  binary_in = TRUE;
	}
      ...
      else if(strncmp(argv[i],"-her",4)==0)
	{
	  *type = 1;
	  if(type_index == -1)
	    ...
	    fprintf(stderr,
		    "\n!!WARNNING!! Det expects real hermitain matrices\n");
...
/* obtain the in files */
  ...
  if(Ain_name==NULL)
    ...
    *Ain=fopen(Ain_name,"r");
    if(*Ain==NULL)
      error("DeterminantInverse: the specified input file could not be accessed");
  }
...
/* obtain the out files */
  ...
  if(det_name==NULL)
	...
	error("DeterminantInverse: the specified determinant output file could not be created");
    }
  ...
  if(cond_name==NULL)
	...
	error("DeterminantInverse: the specified condition output file could not be created");
  }
}
...
void flag_error()
{
  fprintf(stderr,"\n");
  fprintf(stderr,"%s\n",
	  "Det - a program for calculating determinants");
