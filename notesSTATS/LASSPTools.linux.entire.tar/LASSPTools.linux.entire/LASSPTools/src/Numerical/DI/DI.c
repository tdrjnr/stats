/*****************************************************************
 ...
 DI [-real, -complex] [-general, -symmertric, -hermitian
...
#endif
{
  ...
  FILE     *Ain;
  FILE     *detinv_out[3]; /* the first file ptr is to the det out file    *
			    * the second file ptr is to the inv out file   *
			    * the third file ptr is to the cond out file   */
  ...
  get_options(argc, argv, &Ain, detinv_out, &find_detinv,
	      &flag, &matrix_in, &matrix_out, &type);
  ...
  if(type > 0) { /* The section for complex matrices */
      ...
      while(TRUE)
	{
	  ...
	  find_detinv(C, &Z, detinv_out[2], &flag);
	  
	  stop_time=clock();
	  elapsed_time=((float) (stop_time-start_time))/CLOCKS_PER_SEC;
	  fprintf(stderr,"DI: time taken was %f seconds\n"
		  , elapsed_time);
	  
	  if(flag & 1)
	    matrix_out(*C, detinv_out[1]);
	  if(flag & 10) {
	    if(type == 0) /* Remember type-- at the beginning of */
	      {           /* the complex section                 */
		power = floor(Z.work[2]);
		fprintf(detinv_out[0],"%lfe%d+%lfe%dI\n\n",Z.work[0],power,
			Z.work[1],power);
		fflush(detinv_out[0]);	/* Added JPS 4/11/95 */
	      }
	    else
	      {
		power = floor(Z.work[1]);
		fprintf(detinv_out[0],"%lfe%d\n\n",Z.work[0],power);
		fflush(detinv_out[0]);	/* Added JPS 4/11/95 */
	      }
	  }
	}
      ...
      while(TRUE)
	{
	  ...
	  find_detinv(A, &Z, detinv_out[2], &flag);
...
void get_options(int argc, char **argv, FILE **Ain, FILE **detinv_out,
		 DetInvFctn *det_fctn, int *flag, GetMatrix *matrix_in,
		 PutMatrix *matrix_out, int *type)
#else
void get_options(argc, argv, Ain, detinv_out, det_fctn, flag, matrix_in,
		 matrix_out, type)
int argc;
char **argv;
FILE **Ain;
 ...
 ************************/
{
  ...
  char  *cond_name=NULL,*det_name=NULL,*inv_name=NULL;
  int   i;
  int   fact_index;
  int   type_index;
  int   binary_out;
  ...
  *flag = 0;
  ...
  for(i=1; i<argc; i++)
	...
	{ 
	  argv[i]=argv[i]+6;
	  if(strncmp(argv[i],"det",3)==0) {
	    i++;
	    if(i<argc) {
	      if(*argv[i]=='-') flag_error();
	      else det_name=argv[i];
	    }
	    else flag_error();
	  }
	  else if(strncmp(argv[i],"inv",3)==0) {
	    i++;
	    if(i<argc) {
	      if(*argv[i]=='-') flag_error();
      ...
      else if(strncmp(argv[i],"-cond",5)==0)
	{
      ...
      else if(strncmp(argv[i],"-det",4)==0)
	{
	  *flag = *flag | 10;
	}
      ...
      else if(strncmp(argv[i],"-her",4)==0)
	{
	  *type = 1;
	  if(type_index == -1)
	    ...
	    fprintf(stderr,
		    "\n!!WARNNING!! DI expects real hermitain matrices\n");
...
/*---------------------------------------------------------------------------*/
/* set the flag to the requested mode                                     *
 * if *flag ==11 then both the determinant and the inverse are calculated *
 * if *flag ==10 then only the determinant is calculated                  *
 * if *flag == 1 then only the inverse is calculated                      */
...
/* obtain the out files */
  ...
  if(det_name==NULL)
    ...
    if(strcmp(det_name,"stderr")==0)
      detinv_out[0] = stderr;
    else {
      if(!(*flag && 10)) {
	*flag = *flag | 10;
      }
      detinv_out[0]=fopen(det_name,"w");
      if(detinv_out[0]==NULL)
	error("DI: the specified determinant output file could not be created");
    }
  ...
  if(inv_name==NULL)
	...
	*flag = *flag | 1;
      ...
      detinv_out[1]=fopen(inv_name,"w");
      if(detinv_out[1]==NULL)
	error("DI: the specified inverse output file could not be created");
  }

  if(cond_name==NULL)
    detinv_out[2]=default_out;
  else
    if(strcmp(cond_name,"stderr")==0)
      detinv_out[2] = stderr;
    else {
      detinv_out[2]=fopen(cond_name,"w");
      if(detinv_out[2]==NULL)
	error("DI: the specified condition output file could not be created");
  }
}
...
void flag_error()
{
  fprintf(stderr,"\n");
  fprintf(stderr,"%s",
	  "DI - a program for calculating determinants or");
  fprintf(stderr,"%s\n"," inverses");
  ...
  fprintf(stderr,"%s\n",
	  " -file_determinant det_file\t:output the determinants to det_file");
  fprintf(stderr,"%s\n",
  ...
  fprintf(stderr,"%s\n"," -determinant\t:calculate only the determinant");
  fprintf(stderr,"%s\n"," -inverse\t:calculate only the inverse");
