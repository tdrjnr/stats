...
c
      apstrand = mod(float(ix) / 30269. + float(iy) / 30307. +
     +                        float(iz) / 30323., 1.0)
      return
      end
c
...
c
c	Seeds can be set by calling the routine setuniform
c
...
c
	...
	subroutine setuniform(seed1, seed2)
c
...
c
	...
	subroutine setrandom(seed1, seed2, seed3)
c
c 	Set seeds for the first generator
c
	integer*4 ix,iy,iz,seed1,seed2,seed3
	common /randc/ ix,iy,iz
	save /randc/
