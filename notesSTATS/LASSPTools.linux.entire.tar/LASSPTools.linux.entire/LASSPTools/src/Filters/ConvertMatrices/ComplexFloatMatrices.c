 ...
 * This program uses the various get_complex_float_matrix and		*
 * put_complex_float_matrix routines to convert ascii to and from	*
...
/* garbage */
   ...
   V = *create_default_complex_float_matrix();

   if (!binary_in) 
   {
   	while(get_complex_float_matrix(&V, in) != EOF)
	{
	      if (!binary_out) put_complex_float_matrix(V, out);
	      else if (binary_out==1) put_complex_float_binary_matrix(V, out);
	      else if (binary_out==2) 
			put_complex_float_Mathematica_matrix(V, out);
	      else if (binary_out==3)
			put_complex_float_ordered_pair_matrix(V,out);
	      else if (binary_out==4) put_complex_float_simple_matrix(V,out);
	      else if (binary_out==5) put_complex_float_HTML_matrix(V, out);
	      else fprintf(stderr, "Unrecognized output format\n");
   	}
   }
   else
   {
   	while(get_complex_float_binary_matrix(&V,in) != EOF)
	{
	    if (!binary_out) put_complex_float_matrix(V, out);
	    else if (binary_out==1) put_complex_float_binary_matrix(V, out);
	    else if (binary_out==2)
...
help()
{
   fprintf(stderr, "\nConvertFloatMatrix uses the various get_complex_float_matrix and put_complex_float_matrix routines to\n");
   fprintf(stderr, "convert ascii to and from binary, and files to and from pipes\n");
   fprintf(stderr, "\n-binary_in gives binary single precision format for input  \n");
   fprintf(stderr, "    (starting with binary integer for columns)\n");
   fprintf(stderr, "\n-binary_out gives binary single precision format for output\n");
