 ...
 * This program uses the various get_complex_float_vector and		*
 * put_complex_float_vector routines to convert ascii to and from	*
...
/* garbage */
   ...
   V = *create_default_complex_float_vector();

   if (!binary_in) 
   {
   	while(get_complex_float_vector(&V, in) != EOF)
	{
	      if (!binary_out) put_complex_float_vector(V, out);
	      else if (binary_out==1) put_complex_float_binary_vector(V, out);
	      else if (binary_out==2) 
			put_complex_float_Mathematica_vector(V, out);
	      else if (binary_out==3)
			put_complex_float_ordered_pair_vector(V,out);
	      else if (binary_out==4)
			put_complex_float_simple_vector(V,out);
	      else if (binary_out==5) 
			put_complex_float_HTML_vector(V, out);
	      else fprintf(stderr, "Unknown output format\n");
   	}
   }
   else
   {
   	while(get_complex_float_binary_vector(&V,in) != EOF)
	{
	    if (!binary_out) put_complex_float_vector(V, out);
	    else if (binary_out==1) put_complex_float_binary_vector(V, out);
	    else if (binary_out==2)
...
help()
{
   fprintf(stderr, "\nConvertFloatVector uses the various get_complex_float_vector and put_complex_float_vector routines to\n");
