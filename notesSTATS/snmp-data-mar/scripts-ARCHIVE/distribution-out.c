...
#define THRESHOLD        1000 		// Bucket size KbitsPerSec  
#define NUM_BUCKET       20    		// Number of buckets
...
void main(void)
{
  ...
  // Load the series X
  printf("-----------------------------------------------OUTBOUND Traffic Distribution----------------------------\n");
  load_X_array();
  ...
  printf("  Percentage of OUTBOUND data distribution (for %d samples) \n",
  ...
  for (i=0; i<NUM_BUCKET; i++)
  {
    ...
    printf(" %10.3f Kbits TO %8.3f Kbits = %2ld -- %f %% of OUTBOUND Traffic \n",
