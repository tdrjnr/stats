...
void main(void)
{
  ...
  double   Growth_Utilisation_plus_curr_util;
 ...
 // Compute value
  Total_Data_Kbytes_hr = (Total_Trans_hr * Size_Trans) / 1024;
 
 // Compute value
  Total_Data_Kbits_hr = Total_Data_Kbytes_hr * 8;
 ...
 // Compute value
  Growth_Total_Trans_sec = Growth_Total_Trans_hr / 3600;

 // Compute value
  Growth_Total_Data_Kbytes_hr = (Growth_Total_Trans_hr * Size_Trans) / 1024;

 // Compute value
  Growth_Total_Data_Kbits_hr = Growth_Total_Data_Kbytes_hr * 8;

 // Compute value
  Growth_Total_Data_Kbits_sec = Growth_Total_Data_Kbits_hr / 3600;

 // Compute value
  Growth_Total_plus_overhead = Growth_Total_Data_Kbits_sec * 1.2 * 1.1;

// Compute value
  Growth_Utilisation = (Growth_Total_plus_overhead * 1000) / Bandwidth * 100;

 // Compute value
  Growth_Utilisation_plus_curr_util = Growth_Utilisation + (Util_value * 1000 / Bandwidth * 100);
  ...
  printf("- \n");
  printf("-    Peak Total transactions per/hr  = %f \n", Growth_Total_Trans_hr);
  printf("-    Peak Total transactions per/sec  = %f \n", Growth_Total_Trans_sec);
  printf("-    Total data transmitted at peak hr (Kbytes/hr)  = %f \n", Growth_Total_Data_Kbytes_hr);
  printf("-    Total data transmitted at peak hr (Kbits/hr)  = %f \n", Growth_Total_Data_Kbits_hr);
  printf("-    Total data transmitted at peak (Kbits/sec)  = %f \n", Growth_Total_Data_Kbits_sec );
  printf("-    Total data transmitted at peak plus overheads (Kbits/sec)  = %f \n", Growth_Total_plus_overhead );
  printf("-    Added Utilisation  = %f %\n", Growth_Utilisation);
  printf("-    Total Utilisation on LINK  = %f %\n", Growth_Utilisation_plus_curr_util);
  printf("-------------------------------------------------------- \n");
}
