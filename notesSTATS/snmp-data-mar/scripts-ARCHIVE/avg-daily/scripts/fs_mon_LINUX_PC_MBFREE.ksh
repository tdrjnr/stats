	    ...
	    echo $FSLIMIT | grep MB >/dev/null && IN_FILE="MB" \
            ...
            MB)
                ...
                then
                   ...
                   else # ERROR in MB Free value
                      (( FSLIMIT = $FSLIMIT / 1024 )) # To get a display value
                      (( FSSIZE = $FSSIZE / 1024 ))   # To get a dispaly value
                      echo "\nERROR: Invalid filesystem MINIMUM MB FREE specified"
                      echo "       for $FSMOUNT - ${FSLIMIT}MB -- Current size is"
                      echo "       ${FSSIZE}MB.  Exceptions file value must be less"
                      echo "       than or equal to the size of the filesystem"
                      echo "       measured Megabytes\n"
                   fi
                else
            ...
            PC)
                ...
                ;;
            N)  # Testing method not specified, use script defaults

                # Inform the user of this exceptions file error
                ...
                echo "       entry.  Using script defaults...\n"
