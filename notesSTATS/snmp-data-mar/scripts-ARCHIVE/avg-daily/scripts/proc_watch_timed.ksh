...
#          Each event is logged to the file defines by the 
#          $LOGFILE variable. This script also has the ability
#          to execute pre and/or post events. These are controlled
#          by the $RUN_PRE_EVENT and $RUN_POST_EVENT variables. 
#          These variables control execution individually. What ever
#          is to be executed is to be placed in either the
#          "pre_event_script" or "post_event_script" functions, or
#          both. Timing is controlled on the command line.
#
